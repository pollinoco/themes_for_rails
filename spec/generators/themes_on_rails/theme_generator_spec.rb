...
describe ThemesOnRails::Generators::ThemeGenerator do
  ...
  context "default generation" do
    ...
    context "layout file: erb" do
      ...
      it { should exist }
      it { should contain(/\= stylesheet_link_tag\s+\"theme_a\/all\"/) }
      it { should contain(/\= javascript_include_tag\s+\"theme_a\/all\"/) }
      it { should_not contain(/javascript_importmap_tags/) }
    end
  ...
  context "sprockets manifest" do
    before do
      FileUtils.mkdir_p(File.join(destination_root, "app/assets/config"))
      File.write(File.join(destination_root, "app/assets/config/manifest.js"), "//= link_tree ../images\n")
      run_generator %w(theme_a)
    end

    subject { file("app/assets/config/manifest.js") }

    it { should contain("//= link theme_a/all.css") }
    it { should contain("//= link theme_a/all.js") }
  end
end
